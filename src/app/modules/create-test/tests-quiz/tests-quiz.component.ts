...
export class TestsQuizComponent extends Unsubscribe implements OnInit, OnDestroy {
  ...
  public clearQuizTestsIndexDb(): Observable<boolean> {
    return this._indexedDb.clear("quizTests");
  }
  ...
  public confirmStartTest(action: boolean): void {
    ...
    if (action) {
      ...
      this._quizFacade.setSelectedQuizTest(this.testItem.questions);
      queryParams = {
      ...
      this._router.navigate([`/employee/create-test/quiz-test`], {
      ...
      .subscribe((specialist) => {
        specialistPosition.map((position) => {
          if (position.displayName === specialist?.position) {
            ...
            this.searchParams.programmingLanguages = JSON.parse(specialist?.languagesFrameworksForSelect).languages.map(
              (item: { value: string }) => {
                return item.value;
              }
            );
  ...
  public getTest(position: string, searchValue?: SearchParams): Observable<TestInterfaceQuiz | null> {
    if (this.specialistPosition) {
      this.getTestItem = this._createTestFacade.getQuizTest(this.specialistPosition, searchValue).pipe(
        map((tests) => {
