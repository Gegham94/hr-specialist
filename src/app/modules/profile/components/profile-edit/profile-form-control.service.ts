...
import { PatternModel } from "src/app/modules/auth/enum/pattern.model";
...
import { ResumeStateService } from "../utils/resume-state.service";
...
export class ProfileFormControlService {
    ...
    return combineLatest([
      ...
      map(([profile, educations, experiences, skills, resume]) => {
        ...
        if (profile.currency && profile.salary) {
              profile.salary = `${profile.salary[0]["displayName"] ? profile.salary[0]["displayName"] : profile.salary}/${profile.currency[0]["displayName"]
          ? profile.currency[0]["displayName"]
          : profile.currency}`;
        }
  ...
  // public initAll(): Observable<[IProfileInfo | null, IEducation | null, IExperiences | null, IUserSkills | null]> {
  //   return combineLatest([
  //     this.initInfoForm(),
