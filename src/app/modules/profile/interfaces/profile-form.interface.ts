import { SpecialistGenderEnum, SpecialistGenderTypeEnum } from "src/app/modules/profile/enums/specialist-gender.enum";
...
export class ProfileInfo implements Partial<IProfileInfo> {
  ...
  setGenderValue(value: string): this {
    if (value) {
      this.gender =
        this.gender === SpecialistGenderTypeEnum.Male ? SpecialistGenderEnum.Male : SpecialistGenderEnum.Female;
    }
    return this;
  }
...
export interface IEducationItem {
  id?: number;
  education_id: string | ISearchableSelectData[];
  faculty: string | ISearchableSelectData[];
  graduate_date: string;
  trainingFormat: string | ISearchableSelectData[];
...
export interface IUserSkills {
  id?: number;
  programmingLanguages: string | ISearchableSelectData[];
  programmingFrameworks: string | ISearchableSelectData[];
  languages: string | ISearchableSelectData[];
  ...
  constructor(resume: IEmployee) {
    ...
    if (resume.languagesFrameworksForSelect) {
      const languagesAndFrameworks: {
        languages: ISearchableSelectData[];
        frameworks: ISearchableSelectData[];
