...
export class QuizComponent extends Unsubscribe implements OnInit, OnDestroy, CanComponentDeactivate {
  ...
  public isModalOpenConfirm: BehaviorSubject<boolean> = new BehaviorSubject(false);
  ...
  public ngOnInit() {
    this._quizFacade.setShouldDeactivate(false);
    this._quizState
      .getSelectedQuizTest()
      .pipe(
        switchMap((selectedQuiz) => {
          if (!selectedQuiz || selectedQuiz === undefined) {
            return this.indexedDb
              .getByIndex<{ data: IQuizTestResponse[]; settings: IQuizTestResponseSettings; id: number }>(
                "quizTests",
                "id",
                1
              )
              .pipe(
                ...
                switchMap((res) => {
                  ...
                  this._quizState.setSelectedQuizTest(res.data);
                  this.timerStartCount = res.settings.remainingTime ? parseInt(res.settings.remainingTime) : 0;
                  this.timerStartCountFixed = res.settings.remainingFixedTime
                    ? parseInt(res.settings.remainingFixedTime)
                    : 0;
                  return of(res);
                }),
                switchMap(() => {
          ...
          return this.quizSelectedTest$.pipe(
            tap((tests) => {
              if (tests) {
                ...
                this._quizFacade.setShouldDeactivate(true);
  ...
  public confirmResult(action: boolean) {
    if (action) {
      this.quizSelectedTest$
      .pipe(
        takeUntil(this.ngUnsubscribe),
  ...
  public modalToggleConfirm(val?: boolean) {
    this.isModalOpenConfirm.next(val !== undefined ? val : !this.isModalOpenConfirm);
    document.body.style.overflow = "auto";
  }
